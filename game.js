...
function paintWindow() {
  ...
  context_score.fillText("Score :" + "0", 150, 50);
...
function upDate() {
  ...
  context_score.fillStyle = "white";
  context_score.fillText("Score :" + myScore.toString(), 150, 50);
  // context_score.fillText(
  //   "Score :" + myScore.toString(),
  //   a.width / 2 - textWidth / 2 + 9,
  //   50
  // );
    ...
    if (eachState[i]) {
      if (myTiles[i].y < 470 && myTiles[i].y > 350) {
        switch (myTiles[i].keyCode) {
          case 65: //A
            if (myTiles[i].x == 0) afterRight(i);
            break;
          case 83: //S
            if (myTiles[i].x == 75) afterRight(i);
            break;
          case 68: //D
            if (myTiles[i].x == 152) afterRight(i);
            break;
          case 70: //F
            if (myTiles[i].x == 228) afterRight(i);
            break;
        }
      }
      if (myTiles[i].y > 470) {
        ...
        myFails++;
        context_score.clearRect(0, 70, 300, 40);
        context_score.fillStyle = "white";
        context_score.fillText("Fails :" + myFails.toString(), 150, 100);
      }
    } else {
      //not alive
    }
